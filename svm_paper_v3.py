#need to import matplotlib especially plt before snap, otherwise the kernel will die
import matplotlib
matplotlib.use('Agg')
...
#Plotting
import os
mypath = '/home/yan/Documents/git/cs520_network_lasso/NetworkLasso-master'
os.chdir(mypath)    
import tempfile
...
def solveX(data):
	...
	a = cp.Variable((inputs,1))
	epsil = cp.Variable((numtests,1))
	constraints = [epsil >= 0]
	g = c*cp.norm(epsil,1)
	for i in range(inputs - 1):
		g = g + 0.5*cp.square(a[i])
	...
	for i in range(int(neighs.size/(2*inputs+1))):
		...
		if(weight != 0):
			...
			f = f + rho/2*cp.square(cp.norm(a - z.reshape(-1,1) + u.reshape(-1,1)))
	objective = cp.Minimize(50*g + 50*f)
	p = cp.Problem(objective, constraints)
	result = p.solve()
	if(result == None):
		objective = cp.Minimize(50*g+51*f)
		p = cp.Problem(objective, constraints)
		result = p.solve(verbose=False)
		if(result == None):
			print ("SCALING BUG") #CVXOPT scaling issue (rarely occurs)
			objective = cp.Minimize(52*g+50*f)
			p = cp.Problem(objective, constraints)
			result = p.solve(verbose=False)
	return a.value, g.value

#runADMM(G1, sizeOptVar, sizeData, lamb, rho + math.sqrt(lamb), numiters, x, u ,z, trainingSet, edgeWeights, numtests, useConvex, c, epsilon)
...
def runADMM(G1, sizeOptVar, sizeData, lamb, rho, numiters, x, u, z, a, edgeWeights, numtests, useConvex, c, epsilon):
	...
	if(useConvex == 1):
		return (x,u,z,0,0,iters)
	else:
		return (bestx,bestu,bestz,0,0,iters)

def main():

	#Set parameters
	useConvex = 1 #1 = true, 0 = false
	rho = 0.0001#rho is the penalty parameter 
	numiters = 50
	#thresh = -1#10
	thresh = 3
	...
	numtests = 25#Training set size
	testSetSize = 10#Test set size
	...
	for NI in G1.Nodes():
		for NI2 in G1.Nodes():
			if(NI.GetId() < NI2.GetId()):				
				if (round(NI.GetId()/sizepart) == round(NI2.GetId()/sizepart)):
	...
	for EI in G1.Edges():
		...
		adj_mat[temp.GetVal1(),temp.GetVal1()]=1

	#Generate side information
	a_true = np.random.randn(sizeOptVar, partitions)#the true w parameter vector for every partition
	v = np.random.randn(numtests,nodes)#noise for each training sample each node
	vtest = np.random.randn(testSetSize,nodes) #noise for each test sample each node
	np.save(f'w_true',a_true)
	np.save(f'training_noise',v)
	np.save(f'test_noise',vtest)
	...
	for i in range(nodes):
		a_part = a_true[:,math.floor(i/sizepart)]
	...
	[plot1, plot2, plot3,plot4] = [TFltV(), TFltV(), TFltV(), TFltV()]	
	while(lamb <= thresh or lamb == 0):
		(x, u, z, pl1, pl2,iters) = runADMM(G1, sizeOptVar, sizeData, lamb, rho + math.sqrt(lamb), numiters, x, u ,z, trainingSet, edgeWeights, numtests, useConvex, c, epsilon)
		print ("Lambda = ", lamb)
		np.save(f'x_with_lambda_{lamb:4f}',x)
		np.save(f'u_with_lambda_{lamb:4f}',u)
		np.save(f'z_with_lambda_{lamb:4f}',z)
		#Get accuracy
	...
	if(thresh > 0):
		...
		if(useConvex == 1):
			plt.savefig(f'image_svm_convex_edges_{edges}',bbox_inches='tight')
		else:
			plt.savefig(f'image_svm_nonconvex_edges_{edges}',bbox_inches='tight')
		...
		if(useConvex == 1):
			plt.savefig(f'consensus_svm_convex_edges_{edges}',bbox_inches='tight')
			np.savetxt(f'text_svm_convex_edges_{edges}.out', (pl1, pl2, pl3,pl4), delimiter=',', fmt='%1.4f')
		else:
			plt.savefig(f'consensus_svm_nonconvex_edges_{edges}',bbox_inches='tight')
			np.savetxt(f'text_svm_nonconvex_edges_{edges}.out', (pl1, pl2, pl3,pl4), delimiter=',', fmt='%1.4f')
